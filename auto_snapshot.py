...
# Branches this watcher is willing to auto-commit on.  `main` is the curated
# published-artifact branch; auto-snapshots must never land there.  Detached
# HEAD (current_branch == "") is also refused.
...
def commit_if_dirty():
    ...
    if branch not in ALLOWED_BRANCHES:
        log(f"skipping auto-commit: current branch {branch!r} not in {sorted(ALLOWED_BRANCHES)}")
        return
    # Check what changed
    try:
