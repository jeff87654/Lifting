...
LogTo("C:/Users/jeffr/Downloads/Lifting/tmp_h1_section_s18_5553_on.log");
...
ResetH1OrbitalStats();
pack := BuildCombo();
Print("CASE s18_5553_on section=", H1_OUTER_SECTION_ACTION,
      " |P|=", Size(pack[1]), " |N|=", Size(pack[4]), "\n");
t0 := Runtime();
res := HoltFPFSubgroupClassesOfProduct(pack[1], pack[2], pack[3], pack[4]);
elapsed := Runtime() - t0;
Print("RESULT s18_5553_on count=", Length(res), " elapsed_ms=", elapsed, "\n");
PrintH1OrbitalStats();
LogTo();
QUIT;
