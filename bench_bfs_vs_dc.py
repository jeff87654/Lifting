"""bench_bfs_vs_dc.py — head-to-head: BFS vs DoubleCosets on the SAME
BFS-bound matches across multiple RIGHT factors.

LEFT: predict_species_tmp/_h_cache_topt/10/[4,4,2]/[2,1]_[4,3]_[4,3].g
RIGHTs: D_4 (TG(4,3)) — small AutQ; TG(8,22) — mixed (incl. |Aut|=20160)

For each non-saturating (h1orb, h2orb) match:
  - n_orb_bfs, t_bfs_us   (microseconds)
  - n_orb_dc,  t_dc_us
  - check n_orb_bfs == n_orb_dc

Output: bench_bfs_vs_dc_tmp/bench.log
Buckets results by (qsize, |AutQ|, |A1|, |A2|).
"""
...
CACHE_PATH = ROOT / "predict_species_tmp/_h_cache_topt/10/[4,4,2]/[2,1]_[4,3]_[4,3].g"
...
def main():
    sandbox = ROOT / "bench_bfs_vs_dc_tmp"
    ...
    print(f"running BFS vs DC bench...")
    t0 = time.time()
