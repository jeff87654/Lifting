...
NonAbelianComplementsViaAut := function(Q, M_bar, C_Q_M)
    local idx, cf, j, gensT, autT, isoPerm, autPerm, innGens, innT,
          qGens, qImages, phi, A, complsInA, result, K,
          preimg_gens, k, q;

    idx := Size(Q) / Size(M_bar);

    # Guard: safe iff Hom(C_Q_M, M_bar) is trivial.
    # Since M_bar is simple, Hom(C, M_bar) nontrivial iff C has M_bar as
    # a composition factor. Check by scanning composition series.
    if Size(C_Q_M) >= Size(M_bar) then
        cf := CompositionSeries(C_Q_M);
        for j in [1..Length(cf)-1] do
            if Size(cf[j]) / Size(cf[j+1]) = Size(M_bar) then
                return fail;
            fi;
        od;
    fi;
                ...
                if IsSolvableGroup(M_bar) or IsSolvableGroup(Q) then
                    ...
                    complements := CallFuncList(function()
                        local idx, compReps, g, C, c, transversal, t, gens,
                              subC, result, _cf, _hasMatchingFactor, _j, _t0,
                              _autResult;
                        idx := Size(Q) / Size(M_bar);
                        ...
                        if Size(C) = idx and Size(Intersection(C, M_bar)) = 1 then
                            # C is ONE complement. Check if it's the UNIQUE one.
                            # Additional complements exist iff Hom(C, M_bar) is
                            # non-trivial. For simple M_bar, this means C has a
                            # composition factor of the same order as M_bar.
                            _cf := CompositionSeries(C);
                            _hasMatchingFactor := false;
                            for _j in [1..Length(_cf)-1] do
                                if Size(_cf[_j]) / Size(_cf[_j+1]) = Size(M_bar) then
                                    _hasMatchingFactor := true;
                                    break;
                                fi;
                            od;
                            if not _hasMatchingFactor then
                                if layerSize > 60 then
                                    Print("          -> unique centralizer complement\n");
                                fi;
                                return [C];
                            fi;
                            if layerSize > 60 then
                                Print("          -> centralizer complement + diagonals needed\n");
                            fi;
                        fi;
