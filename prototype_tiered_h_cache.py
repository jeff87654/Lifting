"""prototype_tiered_h_cache.py — Build the H-cache for [4,3]^4 LEFT
targeted at C_3 and S_3 RIGHT factors (S19 with M_R=3), using a tiered
enumeration of normal subgroups by Q iso-class.

Tiers per Q-group:
  T1: |Q| prime → abelianization (kernel of H/[H,H] → C_p)
  T2: |Q| <= 200 non-prime → GQuotients(H, Q) and take kernels
  T3: |Q| > 200 → fall through to NormalSubgroups (deferred; not used here)

Compares per-tier timing across the 12,525 FPF subgroups of D_8^4 LEFT.
Produces an H-cache in the standard predict_2factor format that can drive
prediction for combo [4,3]_[4,3]_[4,3]_[4,3]_[3,2] at S19.
"""
import os
import subprocess
...
LOG = ROOT / "prototype_tiered_h_cache.log"
CACHE_OUT = ROOT / "prototype_h_cache_43_4_for_S3.g"
...
PrintTo("{str(CACHE_OUT).replace(chr(92), "/")}",
    "H_CACHE := ", H_CACHE, ";\\n");
Print("saved to {str(CACHE_OUT).replace(chr(92), "/")}\\n");

LogTo();
QUIT;
'''

(ROOT / "prototype_tiered_h_cache.g").write_text(GAP_SCRIPT, encoding="utf-8")

bash_exe = r"C:\Program Files\GAP-4.15.1\runtime\bin\bash.exe"
script_cyg = "/cygdrive/c/Users/jeffr/Downloads/Lifting/prototype_tiered_h_cache.g"

env = os.environ.copy()
env["PATH"] = r"C:\Program Files\GAP-4.15.1\runtime\bin;" + env.get("PATH", "")
env["CYGWIN"] = "nodosfilewarning"

print(f"Running tiered H-cache prototype on [4,3]^4 LEFT...")
print(f"Log: {LOG}")
