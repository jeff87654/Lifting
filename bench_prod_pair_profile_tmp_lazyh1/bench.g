...
for i in [START_IDX..(START_IDX + N_PAIRS - 1)] do
    ...
    for h1_orb_idx in [1..Length(H1data.orbits)] do
        ...
        for h2idx in H2DATA.byqid.(key) do
            ...
            # LAZY_H1_AUT optimization: h2.full_aut is pre-warmed at startup.
            # If h2 saturates, skip EnsureAutQ(h1orb) entirely.
            if LAZY_H1_AUT = 1 and h2orb.full_aut = true then
                n_orb := 1; orbit_reps_phi := [isoTH];
                n_short := n_short + 1;
                # don't add to t_autq or t_bfs — both are zero work
            else
