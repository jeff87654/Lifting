"""Test: verify S2-S10 correctness with small-group fast path (threshold 768)."""
import subprocess
import os
import time
...
log_file = os.path.join(LIFTING_DIR, "test_smallgroup2.log").replace("\\", "/")

gap_code = f'''
LogTo("{log_file}");
Print("Testing S2-S10 with small-group fast path (threshold 768)\\n");
...
script_file = os.path.join(LIFTING_DIR, "test_smallgroup2.g")
...
print(f"Launching S2-S10 test with threshold 768 (timeout 300s)...")
start = time.time()
...
if os.path.exists(log_win):
    with open(log_win, "r") as f:
        for line in f:
            l = line.rstrip()
            if l.startswith("S") or "Expected" in l or "Done" in l:
                print(l)
else:
