"""Test S2-S10 after H^1 fingerprint optimization (removed preimageGens/ambientGroup)."""
import subprocess
import os
import time
...
log_file = "C:/Users/jeffr/Downloads/Lifting/test_fingerprint_opt.log"
...
script_path = os.path.join(LIFTING_DIR, "test_fingerprint_opt.g")
with open(script_path, "w") as f:
    f.write(gap_code)

script_cygwin = "/cygdrive/c/Users/jeffr/Downloads/Lifting/test_fingerprint_opt.g"
...
print("Running S2-S10 with fingerprint optimization...")
start = time.time()
...
with open(os.path.join(LIFTING_DIR, "test_fingerprint_opt.log"), "r") as f:
    log = f.read()

for line in log.split('\n'):
    if any(x in line for x in ['PASS', 'FAIL', 'ALL', 'Total', 'SOME', 'cache', 'Cache', 'H1']):
        print(line.strip())
