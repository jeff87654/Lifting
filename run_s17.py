###############################################################################
# run_s17.py - S17 conjugacy class computation
#
# Computes all 66 FPF partitions of 17 using the current verified code
# (Phase C1 + series stabilizer + Goursat + all optimizations).
#
# Derived from run_s16_fresh.py with:
#   - New output directory: parallel_s17/
#   - Updated cost estimates scaled from actual S16 timing data
#   - 24h timeout per worker
#   - OEIS A000638(17) = 1,466,358 target (FPF = 780,193)
#
# Usage:
#   python run_s17.py --dry-run               # Preview assignment
#   python run_s17.py                          # Launch computation
#   python run_s17.py --workers 4              # Use 4 workers instead of 8
#   python run_s17.py --resume                 # Resume incomplete partitions
#   python run_s17.py --combine-only           # Assemble final cache
#
###############################################################################
...
N = 17
INHERITED_FROM_S16 = 686165  # OEIS A000638(16)
OEIS_S17 = 1466358           # OEIS A000638(17)
EXPECTED_FPF = OEIS_S17 - INHERITED_FROM_S16  # = 780,193

# NrTransitiveGroups for degrees 1..17
NR_TRANSITIVE = {
    1: 1, 2: 1, 3: 2, 4: 5, 5: 5, 6: 16, 7: 7, 8: 50,
    9: 34, 10: 45, 11: 8, 12: 301, 13: 9, 14: 63, 15: 104, 16: 1954, 17: 5,
}

# S17 cost estimates (seconds), derived from actual S16 timing data scaled ~1.3x
# for the degree increase, plus heuristic estimates for new partition shapes.
S17_COST_ESTIMATE = {
    # === Tier 1: 4-10h each (one per worker) ===
    # [4,4,4,4] took 22032s in S16; [5,4,4,4] adds degree-5 factor (5 trans groups)
    (5, 4, 4, 4):     30000,
    # [8,4,4] took 8039s in S16; [9,4,4] has 34 trans groups for degree-9
    (9, 4, 4):        25000,
    # [8,6,2] took 14532s; [9,6,2] has 34*16=544 combos
    (9, 6, 2):        20000,
    # [8,4,2,2] took 8956s; [9,4,2,2] similar structure
    (9, 4, 2, 2):     18000,
    # [6,4,4,2] took 4510s; [7,4,4,2] has 7 trans for degree-7
    (7, 4, 4, 2):     15000,
    # [4,4,4,2,2] took 15150s; [5,4,4,2,2] adds degree-5
    (5, 4, 4, 2, 2):  15000,
    # [4,4,4,4] took 22032s; [4,4,4,3,2] is different shape but many combos
    (4, 4, 4, 3, 2):  12000,

    # === Tier 2: 1-4h each ===
    # [6,6,4] took ~8000s; [7,6,4] has 7*16*5=560 combos
    (7, 6, 4):        10000,
    # [6,4,2,2,2] took ~8000s via W6; [7,4,2,2,2] similar
    (7, 4, 2, 2, 2):  10000,
    # [8,8] took ~5000s (Goursat); [9,8] has 34*50=1700 combos
    (9, 8):           8000,
    # [6,6,4] took ~8000s; [6,6,5] has 16*16*5=1280/2=640 combos
    (6, 6, 5):        8000,
    # [4,4,2,2,2,2] took ~8000s via W7; [5,4,2,2,2,2] adds degree-5
    (5, 4, 2, 2, 2, 2): 8000,
    # [8,5,3] took 699s; [9,5,3] has more combos
    (9, 5, 3):        7000,
    # [8,4,4] took 8039s; [8,5,4] has 50*5*5=1250 combos
    (8, 5, 4):        7000,
    # [6,4,3,3] took ~3500s via W6; [7,4,3,3] adds degree-7
    (7, 4, 3, 3):     6000,
    # [6,4,4,2] took 4510s; [6,5,4,2] has more combos
    (6, 5, 4, 2):     6000,
    # [8,3,3,2] took ~2500s; [9,3,3,2] has 34*2*2=136 combos
    (9, 3, 3, 2):     5500,
    # [8,6,2] took 14532s; [8,7,2] has 50*7=350 combos
    (8, 7, 2):        5000,
    # [6,6,2,2] took ~3000s in W10; [7,6,2,2] adds degree-7
    (7, 6, 2, 2):     5000,
    # [8,5,3] took 699s; [8,6,3] has 50*16*2=1600 combos
    (8, 6, 3):        5000,
    # [5,4,4,3] took 1701s; [5,5,4,3] has symmetry in 5s
    (5, 5, 4, 3):     4000,
    # [5,4,3,2,2] took ~1200s in W24; [6,4,3,2,2] adds more combos
    (6, 4, 3, 2, 2):  4000,
    # [4,4,3,3,2] took ~4000s; [4,4,3,3,3] swaps 2->3
    (4, 4, 3, 3, 3):  4000,
    # [9,4,3] took ~3500s; [10,4,3] has 45*5*2=450 combos
    (10, 4, 3):       3500,
    # [4,4,2,2,2,2] took ~8000s; [4,4,3,2,2,2] swaps a 2 for 3
    (4, 4, 3, 2, 2, 2): 3500,
    # [12,4] took ~4500s; [13,4] has 9*5=45 combos
    (13, 4):          3000,
    # [6,3,3,2,2] took ~1500s; [7,3,3,2,2] has 7*2*2=28 combos
    (7, 3, 3, 2, 2):  3000,
    # [8,2,2,2,2] took ~3000s; [9,2,2,2,2] has 34 combos
    (9, 2, 2, 2, 2):  3000,
    # [5,5,4,2] took ~1500s; [5,5,4,2] same shape at degree-17 (new: [6,5,3,3])
    (6, 5, 3, 3):     2500,
    # [5,3,3,3,2] took ~800s; [6,3,3,3,2] adds degree-6
    (6, 3, 3, 3, 2):  2500,

    # === Tier 3: 15min-1h ===
    # [12,4] took 4500s; [12,5] has 301*5=1505 combos
    (12, 5):          2000,
    (10, 4, 2, 2):    2000,
    (10, 7):          2000,   # 45*7=315 combos, Goursat
    (10, 5, 2):       1800,
    (8, 5, 2, 2):     1500,
    (5, 5, 3, 2, 2):  1500,
    (8, 3, 2, 2, 2):  1500,
    ...
    (7, 5, 3, 2):     1500,
    (6, 5, 2, 2, 2):  1200,
    (7, 7, 3):        1200,   # 28*2=56 combos, Goursat
    (7, 5, 5):        1000,   # 7*15=105 combos
    (5, 3, 3, 2, 2, 2): 1000,
    (8, 3, 3, 3):      800,
    (11, 4, 2):        800,
    (11, 6):           800,   # 8*16=128 combos, Goursat
    (9, 3, 2, 2, 2):   800,
    (5, 5, 2, 2, 2, 2): 800,
    (7, 3, 2, 2, 2, 2): 700,
    ...
    # === Tier 4: <15 min ===
    (6, 3, 2, 2, 2, 2): 600,
    (9, 5, 3):        600,    # already listed above, skip duplicate
    (14, 3):          500,
    (13, 2, 2):       500,
    (12, 3, 2):       500,
    (4, 3, 3, 3, 2, 2): 400,
...
MASTER_LOG = os.path.join(OUTPUT_DIR, "run_s17.log")
...
def estimate_partition_cost(partition):
    """Estimate cost of a partition based on S16 timing data scaled for S17.

    Uses S17_COST_ESTIMATE when available, otherwise falls back to
    combo-count heuristic.
    """
    pt = tuple(partition)

    # Direct lookup in S17 cost estimates (most accurate)
    if pt in S17_COST_ESTIMATE:
        return S17_COST_ESTIMATE[pt]
...
def create_manifest(partitions, assignments):
    ...
    manifest = {
        "n": N,
        "inherited": INHERITED_FROM_S16,
        "expected_fpf": EXPECTED_FPF,
        "expected_total": OEIS_S17,
...
def print_assignment(workers, worker_loads, partitions):
    ...
    print(f"  Expected total:  {OEIS_S17}")
...
def launch_gap_worker(script_file, worker_id):
    ...
    # CREATE_NEW_PROCESS_GROUP prevents parent signals from killing workers
...
def get_completed_partitions_from_results(output_dir, max_workers):
    ...
    # Scan all worker result files (auto-detect max worker ID)
...
# ===========================================================================
# Combine results into s17_subgroups.g
# ===========================================================================
...
def parse_inherited_chunked(filepath):
    """Parse s16_subgroups.g into list of generator image lists using chunked parsing."""
...
def combine_results():
    """Combine inherited S16 classes + FPF partition classes into s17_subgroups.g."""
    s16_file = os.path.join(CONJUGACY_CACHE, "s16_subgroups.g")
    s17_file = os.path.join(CONJUGACY_CACHE, "s17_subgroups.g")

    print(f"\nCombining results into {s17_file}...")

    # Step 1: Parse inherited S16 classes
    print(f"  Step 1: Parsing inherited S16 classes from {s16_file}...")
    inherited = parse_inherited_chunked(s16_file)
    print(f"  Loaded {len(inherited)} inherited classes")

    if len(inherited) != INHERITED_FROM_S16:
        print(f"  WARNING: Expected {INHERITED_FROM_S16} inherited classes, got {len(inherited)}")

    # Step 2: Extend inherited generators to degree 17 (add fixed point 17)
    ...
    if total == OEIS_S17:
        print(f"  MATCH: Total {total} == OEIS A000638(17) = {OEIS_S17}")
    else:
        print(f"  MISMATCH: Total {total} != OEIS A000638(17) = {OEIS_S17} (diff: {total - OEIS_S17})")

    # Step 5: Write output
    print(f"  Step 5: Writing {s17_file}...")
    write_subgroups_file(s17_file, all_subgroups, N)
    print(f"  Done! Output: {s17_file}")
    print(f"  File size: {os.path.getsize(s17_file) / 1024 / 1024:.1f} MB")
...
def _scan_checkpoint_progress(incomplete_partitions):
    ...
    for p in incomplete_partitions:
        ...
        log_name = f"ckpt_17_{partStr}.log"
...
def resume_computation(args):
    ...
    # Scan ALL existing checkpoint worker dirs (not just manifest) to find max ID,
    # since manual/external runs may have created dirs beyond manifest tracking.
...
def _recover_checkpoint_logs(incomplete_partitions, active_assignments, next_worker_id):
    """Copy best existing checkpoint .g and .log files to new worker checkpoint dirs.

    The .g file is the monolithic base (all groups + inv keys).
    The .log file is the delta (combos completed since the last .g save).
    Both are needed for correct checkpoint recovery.
    """
    ckpt_base = os.path.join(OUTPUT_DIR, "checkpoints")
    ...
    # Scan all existing worker checkpoint dirs for .g and .log files
    recovered_log = 0
    recovered_g = 0
    for p in incomplete_partitions:
        pt = tuple(p)
        ...
        # Find best .g file (largest, as it has the most data)
        if not (os.path.exists(new_g) and os.path.getsize(new_g) > 0):
            ...
            if best_g and best_g_size > 0:
                ...
                print(f"  RECOVER .g: {list(pt)} - {best_g_size//1024}KB from {os.path.basename(os.path.dirname(best_g))}")
                recovered_g += 1
        ...
        if not (os.path.exists(new_log) and os.path.getsize(new_log) > 0):
            ...
            if best_log and best_combos > 0:
                ...
                print(f"  RECOVER .log: {list(pt)} - {best_combos} combos from {os.path.basename(os.path.dirname(best_log))}")
                recovered_log += 1

    if recovered_g > 0 or recovered_log > 0:
        print(f"  Recovered: {recovered_g} .g files, {recovered_log} .log files")
...
def print_final_results(max_worker_id):
    ...
    print(f"  Inherited from S_{N-1}: {INHERITED_FROM_S16}")
    print(f"  FPF partition classes:  {total_fpf}")
    print(f"  TOTAL:                  {total}")

    if total == OEIS_S17:
        print(f"  MATCH: {total} == OEIS A000638(17) = {OEIS_S17}")
    elif len(partition_counts) == 66:
        print(f"  MISMATCH: {total} != OEIS A000638(17) = {OEIS_S17} (diff: {total - OEIS_S17})")
    else:
        print(f"  ({len(partition_counts)}/66 partitions completed, target: {OEIS_S17})")
...
def main():
    parser = argparse.ArgumentParser(
        description="S17 conjugacy class computation")
    ...
    parser.add_argument("--resume-partitions", nargs="*", default=None,
                       help='Resume specific partitions only, e.g. "[9,8]" "[13,4]"')
    ...
    if total == OEIS_S17:
        log_msg(f"SUCCESS: Matches OEIS A000638(17) = {OEIS_S17}")
    elif len(partition_counts) == 66:
        log_msg(f"MISMATCH: {total} != OEIS A000638(17) = {OEIS_S17}")
    ...
    if n_completed < n_total:
        ...
        print(f"\nRun 'python run_s17.py --combine-only' to assemble the final cache.")
