...
predictors (predict_2factor.py, predict_full_general_wreath.py) plus the
legacy C_2^n linear-algebra fast path.  Output goes to parallel_sn_v2/<n>/
...
def run_combo(n, partition, combo, output_path, log_path, force=False,
    ...
    if route_name in ("distinguished", "holt_split", "burnside_m2"):
        result = run_predictor("predict_2factor.py", combo_str, output_path,
...
def main():
    ...
    ap.add_argument("--out", default=str(ROOT / "parallel_sn_v2"),
                    help="output root (parallel_sn_v2/)")
    ap.add_argument("--h-cache", default=str(ROOT / "predict_species_tmp" / "_h_cache_v2"),
                    help="fresh H-cache directory")
    ap.add_argument("--predictor-tmp",
                    default=str(ROOT / "predict_species_tmp" / "_two_factor_v2"),
    ...
    for n in range(args.n_min, args.n_max + 1):
        ...
        # predict_2factor.py can batch them in a single GAP session.  C_2 fast
        # path and Wreath-RA combos remain per-combo.
        # Group key: (mode_for_predict_2factor, left_combo_tuple)
        from predict_2factor import resolve_inputs as _resolve_inputs
        ...
        def flush_super_pack():
            ...
            cmd = [sys.executable, "predict_2factor.py",
        ...
        def _split_largest_super_batch():
            ...
            new_a = ("super_batch",
                     ...
                     [sys.executable, "predict_2factor.py",
