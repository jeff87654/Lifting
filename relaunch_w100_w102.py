"""Relaunch W100 and W102 with fixed checkpoint resume."""
import subprocess
import os
import time
...
workers = [
    {"id": 100, "partition_gap": "[8,8]", "partition_str": "8_8"},
    {"id": 102, "partition_gap": "[4,4,4,4]", "partition_str": "4_4_4_4"},
]
...
for w in workers:
    ...
    gap_script = (
        ...
        f'Print("Worker {wid} RESTARTED (Phase C1 + merged ckpt) at ", StringTime(Runtime()), "\\n");\n'
        ...
        f'PrintTo("{PARALLEL}/worker_{wid}_heartbeat.txt", "starting {part_gap} (merged ckpt)\\n");\n'
        f'partStart := Runtime();\n'
...
print("\nDone. Check heartbeat files for progress.")
