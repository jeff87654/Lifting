...
LogTo("C:/Users/jeffr/Downloads/Lifting/tmp_h1_section_s18_5553_off.log");
...
BuildCombo := function()
  ...
  partition := [5,5,5,3];
  currentFactors := [TransitiveGroup(5,5), TransitiveGroup(5,5), TransitiveGroup(5,5), TransitiveGroup(3,2)];
  shifted := [];
  offs := [];
  off := 0;
  ...
  Nfull := BuildConjugacyTestGroup(18, partition);
...
ResetH1OrbitalStats();
pack := BuildCombo();
Print("CASE s18_5553_off section=", H1_OUTER_SECTION_ACTION,
      " |P|=", Size(pack[1]), " |N|=", Size(pack[4]), "\n");
t0 := Runtime();
res := HoltFPFSubgroupClassesOfProduct(pack[1], pack[2], pack[3], pack[4]);
elapsed := Runtime() - t0;
Print("RESULT s18_5553_off count=", Length(res), " elapsed_ms=", elapsed, "\n");
PrintH1OrbitalStats();
LogTo();
QUIT;
