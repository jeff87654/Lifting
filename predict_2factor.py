...
# Q-sizes the LEFT cache must cover when consumed against a RIGHT factor of
# degree M_R.  For M_R >= 6 the union of subgroup orders of TG(M_R,*) already
# spans most divisors of typical |H|, so just compute everything.
RequiredQSizes := function(M_R)
    local sizes, t, T, K;
    if M_R = 0 then return [1]; fi;
    if M_R >= 6 then return fail; fi;
    sizes := Set([1]);
    for t in [1..NrTransitiveGroups(M_R)] do
        T := TransitiveGroup(M_R, t);
        for K in NormalSubgroups(T) do
            AddSet(sizes, Size(T) / Size(K));
        od;
    od;
    return sizes;
end;

UnionQSizes := function(a, b)
    if a = fail or b = fail then return fail; fi;
    return Union(a, b);
end;

QSizesMissing := function(have, want)
    if have = fail then return []; fi;
    if want = fail then return fail; fi;
    return Filtered(want, s -> not (s in have));
end;

# Backward compat: legacy entries lacking computed_q_sizes are full-compute.
NormalizeHCacheEntry := function(entry)
    if not IsBound(entry.computed_q_sizes) then
        entry.computed_q_sizes := fail;
    fi;
    return entry;
end;

# Enumerate normal subgroups K of H (K ≠ H) with |H/K| in q_size_filter.
# Uses an abelianization fast path for q_size_filter ⊆ {1, 2} that avoids
# the full NormalSubgroups(H) lattice — critical for highly-symmetric H
# like D_8^4 where NormalSubgroups can take many minutes.  For other
# filters falls back to NormalSubgroups + Filtered.
_EnumerateNormalsForSizes := function(H, q_size_filter)
    local primary, DH, hom, A;
    if q_size_filter = fail then
        return Filtered(NormalSubgroups(H), K -> K <> H);
    fi;
    primary := Filtered(q_size_filter, s -> s <> 1);
    if Length(primary) = 0 then return []; fi;
    if primary = [2] then
        # Index-2 fast path: K of index 2 in H ↔ index-2 subs of H/[H,H]
        DH := DerivedSubgroup(H);
        if Index(H, DH) = 1 then return []; fi;   # H is perfect
        hom := NaturalHomomorphismByNormalSubgroup(H, DH);
        A := Range(hom);
        return List(Filtered(MaximalSubgroupClassReps(A), K -> Index(A, K) = 2),
                    K -> PreImage(hom, K));
    fi;
    # General case: enumerate full lattice and filter
    return Filtered(NormalSubgroups(H),
                    K -> K <> H and Size(H) / Size(K) in q_size_filter);
end;

# Decompose a list of normal-subgroup K's into N_H-orbits and build
# the orbit records (one per orbit rep).
...
ComputeHCacheEntry := function(H, S_M, q_size_filter)
    local N_H, normals;
    N_H := Normalizer(S_M, H);
    normals := _EnumerateNormalsForSizes(H, q_size_filter);
    return rec(
...
# Add new orbit records to `entry` covering additional Q-sizes (or all
# remaining sizes when additional_q_sizes = fail).  Reuses cached N_H.
ExtendHCacheEntry := function(entry, S_M, additional_q_sizes)
    local H, N_H, current, normals, new_orbits;
    if entry.computed_q_sizes = fail then return entry; fi;
    H := SafeGroup(entry.H_gens, S_M);
    N_H := SafeGroup(entry.N_H_gens, S_M);
    current := entry.computed_q_sizes;
    if additional_q_sizes = fail then
        # Going to full coverage: enumerate all normals not already covered
        normals := Filtered(NormalSubgroups(H),
                            K -> K <> H and not (Size(H) / Size(K) in current));
    else
        additional_q_sizes := Filtered(additional_q_sizes, s -> not (s in current));
        if Length(additional_q_sizes) = 0 then return entry; fi;
        normals := _EnumerateNormalsForSizes(H, additional_q_sizes);
    fi;
    ...
    if additional_q_sizes = fail then
        ...
        entry.computed_q_sizes := Union(current, additional_q_sizes);
    fi;
    return entry;
end;
...
LEFT_Q_FILTER := RequiredQSizes(MR);   # what this LEFT cache must cover
Print("LEFT q-size filter for M_R=", MR, ": ", LEFT_Q_FILTER, "\n");
...
if H_CACHE <> fail then
    ...
    for hi in [1..Length(H_CACHE)] do
        missing := QSizesMissing(H_CACHE[hi].computed_q_sizes, LEFT_Q_FILTER);
...
if H_CACHE = fail then
    ...
    for hi in [1..Length(SUBGROUPS_LEFT_RAW)] do
        ...
        Add(H_CACHE, ComputeHCacheEntry(SUBGROUPS_LEFT_RAW[hi], W_ML, LEFT_Q_FILTER));
    od;
    if CACHE_LEFT_PATH <> "" then
        SaveHCacheList(CACHE_LEFT_PATH, H_CACHE);
    fi;
...
for job_idx in [1..Length(JOBS)] do
    LEFT_Q_FILTER := UnionQSizes(LEFT_Q_FILTER, RequiredQSizes(JOBS[job_idx].m_right));
od;
Print("[t+", Runtime() - batch_t0, "ms] LEFT q-size filter: ",
      LEFT_Q_FILTER, "\n");
...
for group_idx in [1..Length(GROUPS)] do
    ...
    for hi in [1..Length(GROUP.jobs)] do
        LEFT_Q_FILTER := UnionQSizes(LEFT_Q_FILTER, RequiredQSizes(GROUP.jobs[hi].m_right));
    od;
    Print("  [t+", Runtime() - group_t0, "ms] LEFT q-size filter: ",
          LEFT_Q_FILTER, "\n");
