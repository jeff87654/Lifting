LogTo("C:/Users/jeffr/Downloads/Lifting/holt_engine/tests/verify_big_d4_by_lifting.log");
...
HOLT_ENGINE_MODE := "legacy";  # full legacy pipeline

# Unbind D4_CUBE_CACHE so FAST PATH 5 (D_4^3) is skipped; legacy falls to
# general lifting. Keeps FAST PATHS 1-4 intact (small abelian / small CCS /
# Goursat 2-factor — none triggered for this combo).
if IsBound(D4_CUBE_CACHE) then Unbind(D4_CUBE_CACHE); fi;
...
Print("D_4^3 cache unbound: path will use general lifting\n\n");
...
Print("Disk count (via Goursat+D_4^3 path): ", disk, "\n\n");

Print("Running FindFPFClassesByLifting (general lifting, no D_4^3)...\n");
t0 := Runtime();
result := FindFPFClassesByLifting(P, shifted, offs, Npart);
t := (Runtime() - t0)/1000.0;
Print("Lifting count: ", Length(result), " in ", t, "s\n");
