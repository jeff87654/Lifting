"""M3 S_11 checkpoint: CountAllConjugacyClassesFast(11) should = 3094.
Uses cached S_1..S_10 from lift_cache.g so only S_11 is computed fresh.
"""

import os, subprocess, sys, time

LIFTING_DIR = r"C:\Users\jeffr\Downloads\Lifting"
LOG_FILE = os.path.join(LIFTING_DIR, "holt_engine", "tests", "m3_s11_log.txt")
...
# Clear LIFT_CACHE.11 so we force S_11 computation; keep S_1..S_10
...
QUIT;
'''
    cmd_file = os.path.join(LIFTING_DIR, "holt_engine", "tests", "temp_m3_s11.g")
...
if __name__ == "__main__":
    print("=== Running M3 S_11 checkpoint ===")
    ...
    print(f"GAP runtime: {gap_elapsed}s (legacy baseline ~38s)")
    print(f"[{'PASS' if ok else 'FAIL'}] M3 S_11 checkpoint")
