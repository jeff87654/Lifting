...
log_file = "C:/Users/jeffr/Downloads/Lifting/gap_output_phase3.log"
...
print(f"Starting Phase 3 test at {time.strftime('%H:%M:%S')}...")
...
try:
    with open(r"C:\Users\jeffr\Downloads\Lifting\gap_output_phase3.log", "r") as f:
        log = f.read()
    print("=== GAP Output (key lines) ===")
    for line in log.split('\n'):
        if any(k in line for k in ['Partition', 'Time:', 'Total', 'total', 'count:',
                                     'expected', 'CROSS_VALIDATE', 'cleared', '===',
                                     'LiftThroughLayer', 'time:', 'S8', 'S9', 'S10']):
            print(line)
