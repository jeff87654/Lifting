"""bench_bfs_vs_dc_monster.py — bench D_4 RIGHT against the LEFT cache for
the 3040s combo (n=16 [4,4,4,4] [4,3]^4), which is the monster D_4-pivot combo.

LEFT for that combo: n=12/[4,4,4]/[4,3]_[4,3]_[4,3].g (LEFT removes one [4,3])
"""
...
CACHE_PATH = ROOT / "predict_species_tmp/_h_cache_topt/12/[4,4,4]/[4,3]_[4,3]_[4,3].g"
...
Print("=== bench_bfs_vs_dc_monster (LEFT=[4,3]^3, RIGHT=D_4) ===\n");
...
# Build RIGHT = D_4
S_MR := SymmetricGroup(4);
T_R := TransitiveGroup(4, 3);
...
for idx in indices do
    ...
    for h1orb in h1data.orbits do
        ...
        for h2idx in H2DATA.byqid.(key) do
            ...
            if bfs_res.n_orb <> dc_res.n_orb then n_mismatch := n_mismatch + 1; fi;
...
Print("\n  sampled ", Length(indices), " H1 entries\n");
...
for bucket_key in RecNames(sums) do
    ...
    if bucket.mismatches > 0 then Print("  MISMATCHES=", bucket.mismatches); fi;
    Print("\n");
od;
Print("\n=== done ===\n");
LogTo();
...
def main():
    sandbox = ROOT / "bench_bfs_vs_dc_monster_tmp"
    ...
    print("running monster-D_4 bench...")
    t0 = time.time()
