"""Test GeneralAutHomComplements vs NSCR on diverse small cases.

Covers:
  - direct-product cases (matches HomBasedCentralizerComplements)
  - gcd=1 non-direct-product cases (matches NonAbelianComplementsViaAut)
  - gcd>1 non-direct-product cases (NEW territory)

For each case, counts must match NSCR exactly. If any mismatch, we know the
equivariant-only implementation is incomplete and we need to add the tau
cocycle correction.
"""
...
with open(r"C:\Users\jeffr\Downloads\Lifting\tmp_gen_hom.g", "w") as f:
    f.write(code)
...
p = subprocess.Popen(
    [r"C:\Program Files\GAP-4.15.1\runtime\bin\bash.exe", "--login", "-c",
     'cd "/cygdrive/c/Program Files/GAP-4.15.1/runtime/opt/gap-4.15.1" && ./gap.exe -q -o 0 "/cygdrive/c/Users/jeffr/Downloads/Lifting/tmp_gen_hom.g"'],
...
log = open(r"C:\Users\jeffr\Downloads\Lifting\general_hom_test.log").read()
in_test = False
