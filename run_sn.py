###############################################################################
# run_sn.py - Parallel per-combo conjugacy class computation for S_n
#
# Generalization of run_s18.py: --n selects the target degree. Output lands
# in parallel_s<N>/[partition]/[combo].g, matching the S18 per-combo layout.
#
# Known totals (OEIS A000638): S15=159129, S16=686165, S17=1466358, S18=?
# Inherited-from-prev is read from LIFT_CACHE when available; otherwise must
# be supplied via --inherited.
#
# Usage:
#   python run_sn.py --n 15                  # Compute S15 (regression gate)
#   python run_sn.py --n 17                  # Rerun S17
#   python run_sn.py --n 18 --resume         # Resume S18 production
#   python run_sn.py --n 15 --dry-run        # Preview assignment
#   python run_sn.py --n 15 --workers 6
#   python run_sn.py --n 15 --combine-only   # Assemble final cache
#
# Key features (inherited from run_s18.py):
#   - Per-combo output files in parallel_s<N>/[partition]/
#   - -o 0 for unlimited GAP memory (prevents silent OOM truncation)
#   - Checkpoint + delta log per combo for crash recovery
#   - Reconstruction from combo files if gens file is truncated
#   - Worker respawn on OOM/crash
#   - 7-day per-worker timeout (effectively none)
#
###############################################################################
...
# ===========================================================================
# Configuration — N is set by --n at runtime via main()
# ===========================================================================
...
# Set by main() via --n
N = None
INHERITED_FROM_PREV = None       # OEIS A000638(N-1)
OEIS_TARGET = None               # OEIS A000638(N) - None means unknown
EXPECTED_FPF = None

# Known OEIS A000638 values
OEIS_A000638 = {
    1: 1, 2: 2, 3: 4, 4: 11, 5: 19, 6: 56, 7: 96,
    8: 296, 9: 554, 10: 1593, 11: 3094, 12: 10723,
    13: 20832, 14: 75154, 15: 159129, 16: 686165,
    17: 1466358,  # 18: unknown
}
...
# OUTPUT_DIR et al. are set by main() once N is known. Declared here so
# references in module-level helpers don't NameError on import.
OUTPUT_DIR = None
MANIFEST_FILE = None
GENS_DIR = None
MASTER_LOG = None
...
def _configure_paths(n):
    """Set module-level path globals once we know N."""
    ...
    MASTER_LOG = os.path.join(OUTPUT_DIR, f"run_s{n}.log")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="S_n conjugacy class computation "
                    "(per-combo output, -o 0)")
    parser.add_argument("--n", type=int, required=True,
                       help="Target degree (e.g. 15, 17, 18)")
    parser.add_argument("--inherited", type=int, default=None,
                       help="Override INHERITED_FROM_PREV (OEIS A000638(n-1)); "
                            "defaults to OEIS_A000638 lookup")
    ...
    N = args.n
    if args.inherited is not None:
        INHERITED_FROM_PREV = args.inherited
    elif (N - 1) in OEIS_A000638:
        INHERITED_FROM_PREV = OEIS_A000638[N - 1]
    else:
        parser.error(f"OEIS A000638({N-1}) not known; pass --inherited")
    OEIS_TARGET = OEIS_A000638.get(N)
    EXPECTED_FPF = (OEIS_TARGET - INHERITED_FROM_PREV) if OEIS_TARGET else None
    _configure_paths(N)
    ...
    if n_completed < len(manifest["partitions"]):
        ...
        print(f"\nRun 'python run_sn.py --n {N} --combine-only' to assemble "
              f"the final cache.")
