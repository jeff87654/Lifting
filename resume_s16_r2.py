"""Resume S16 round 2: relaunch dead workers W20 and W25.

Status:
  - W20 dead: [8,8] has checkpoint at combo ~974/1275 (2.8MB checkpoint)
  - W25 dead: [6,5,5] was stuck 16+ hours on combo #201 (hang, not crash)
    Plus 7 more partitions never started

  - W21 alive: [8,6,2] combo 799/800 (about to finish)
  - W22 alive: [8,4,4] combo 515/625 (progressing)
  - W24 alive: on 3rd partition, slow but working

Launch 3 new workers:
  W30: [8,8] (resume from W20 checkpoint)
  W31: [6,5,5], [7,5,4], [7,4,3,2] (heaviest W25 partitions)
  W32: [11,3,2], [6,2,2,2,2,2], [5,5,3,3], [4,4,2,2,2,2], [3,3,3,3,2,2] (lighter)
"""
...
WORKERS = {
    30: {
        "partitions": [(8, 8)],
        "checkpoint_from": 20,  # Copy checkpoint from W20
    },
    31: {
        "partitions": [(6, 5, 5), (7, 5, 4), (7, 4, 3, 2)],
    },
    32: {
        "partitions": [(11, 3, 2), (6, 2, 2, 2, 2, 2), (5, 5, 3, 3),
...
def create_gap_script(partitions, worker_id):
    ...
    gap_code = f'''
LogTo("{log_file}");
Print("Worker {worker_id} (round 2) starting at ", StringTime(Runtime()), "\\n");
...
def main():
    ...
    for wid, info in WORKERS.items():
        ...
        # Copy checkpoint if needed
        if "checkpoint_from" in info:
            old_wid = info["checkpoint_from"]
            for part in parts:
                part_str = "_".join(str(x) for x in part)
                old_file = os.path.join(OUTPUT_DIR, "checkpoints", f"worker_{old_wid}", f"ckpt_{N}_{part_str}.g")
                new_file = os.path.join(ckpt_dir, f"ckpt_{N}_{part_str}.g")
                if os.path.exists(old_file):
                    if not os.path.exists(new_file) or os.path.getmtime(old_file) > os.path.getmtime(new_file):
                        shutil.copy2(old_file, new_file)
                        size_kb = os.path.getsize(new_file) / 1024
                        print(f"  Copied checkpoint for {list(part)}: {size_kb:.0f}KB")
    ...
    print(f"\nDone. Monitor with: for wid in 30 31 32; do cat parallel_s16/worker_${{wid}}_heartbeat.txt; done")
    return 0
