...
predictors (predict_2factor_gq.py, predict_full_general_wreath.py) plus the
legacy C_2^n linear-algebra fast path.  Output goes to parallel_sn_v3/<n>/
...
def run_combo(n, partition, combo, output_path, log_path, force=False,
    ...
    if route_name in ("distinguished", "holt_split", "burnside_m2"):
        result = run_predictor("predict_2factor_gq.py", combo_str, output_path,
...
def main():
    ...
    ap.add_argument("--out", default=str(ROOT / "parallel_sn_v3"),
                    help="output root (parallel_sn_v3/)")
    ap.add_argument("--h-cache", default=str(ROOT / "predict_species_tmp" / "_h_cache_v3"),
                    help="fresh H-cache directory")
    ap.add_argument("--predictor-tmp",
                    default=str(ROOT / "predict_species_tmp" / "_two_factor_v3"),
    ...
    for n in range(args.n_min, args.n_max + 1):
        ...
        # predict_2factor_gq.py can batch them in a single GAP session.  C_2 fast
        # path and Wreath-RA combos remain per-combo.
        # Group key: (mode_for_predict_2factor, left_combo_tuple)
        from predict_2factor_gq import resolve_inputs as _resolve_inputs
        ...
        # Cap on # GROUPS per super-batch (independent of job count).  Each
        # group costs one H-cache build + ReconstructHData inside the GAP
        # session — those are serial within the super-batch, so packing too
        # many groups starves parallelism.  Default to num_workers so each
        # worker gets at most ~1 super-batch's worth of groups.
        ...
        def flush_super_pack():
            ...
            cmd = [sys.executable, "predict_2factor_gq.py",
        ...
        for left_combo, group_jobs in batch_groups.items():
            ...
            if sb_jobs > 1 and n_jobs <= sb_threshold:
                ...
                # Flush when EITHER the job count cap (sb_jobs) OR the group
                # count cap (sb_max_groups) is reached.  The group cap prevents
                # 40+ tiny groups from getting serialized into one worker.
                if (super_pack_total_jobs >= sb_jobs
                        or len(super_pack_groups) >= sb_max_groups):
                    flush_super_pack()
                continue
        ...
        def _split_largest_super_batch():
            ...
            new_a = ("super_batch",
                     ...
                     [sys.executable, "predict_2factor_gq.py",
        ...
        if n in A000638 and n - 1 in A000638:
            ...
            print(f"[n={n}] FPF total: {n_fpf}  expected: {expected_fpf}  "
                  f"{'OK' if ok else 'MISMATCH'}  ({n_seconds:.1f}s, {n_combos} combos)")
        else:
            print(f"[n={n}] FPF total: {n_fpf}  (no OEIS reference)  "
                  f"({n_seconds:.1f}s, {n_combos} combos)")
